...
int main()
{
	...
	while(1){
		...
		GPIO_PTD_GPIO = Number_0;
		delay1(10000);

		if(tmp == 1){
			GPIO_PTD_GPIO = Number_1;
			delay1(10000);
		}
		...
		if(tmp == 4){
			GPIO_PTD_GPIO = Number_4;
			delay1(10000);
		}

		if(tmp == 8){
			GPIO_PTD_GPIO = Number_8;
			delay1(10000);
		}
/*
		if(tmp==1){
			for (j=0; j<8; j++){
				for (myi=0; myi<10000; myi++)
					for (i=0; i<=j; i++){
						GPIO_PTD_GPIO =student[j-i];
			...
			for(x=0;x<8;x++){
				for (j=7; j>=x&&j<8; j--){
					for (myi=0; myi<10000; myi++){
						for (i=7; i>=0&&i<8; i--){
							if(i==j)
								GPIO_PTD_GPIO = student[7-x];
							else if(i<x)
								GPIO_PTD_GPIO = student[7-i];
							else
								GPIO_PTD_GPIO = ~(Number_8 | Number_Dot);
						GPIO_PTA_GPIO = index_7LED[i];
						delay1(10);
