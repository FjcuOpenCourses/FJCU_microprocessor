...
int main()
{
	unsigned int i,sample,sample1,tempt;
	...
	for (i=0; i<3; i++)
		...
		for(sample =0;sample <=0xffff;sample=(sample<<1)+1){
			GPIO_PTB_GPIO = sample;
			delay1(200000);
		}
		for(sample1 =0xffff;sample1>0;sample1 = sample1 >> 1){
					GPIO_PTB_GPIO = sample1;
					delay1(200000);
				}
	}
	for (i=0; i<20; i++)
	...
	for(i=0;i<10;i++){
		...
		GPIO_PTB_GPIO = sample;
		delay1(200000);
		sample=(sample>>1)+0x8000;
		}
		sample=0xfffffff8;
		
		while(sample!=0xffff1fff){
			GPIO_PTB_GPIO = sample;
			delay1(200000);
			sample=(sample<<1)+0x00000001;
		}
	}
