...
int main()
{
	...
	GPIO_PTC_DIR = 0xFFFF;
	GPIO_PTC_CFG = 0x0000;
	unsigned int col,count=0,myi,sw,tmp1;
	unsigned int a,b,i,set,key;
	char opt='a';
	unsigned int index_7LED_NUM[17] = {Number_1, Number_2, Number_3, Number_4, Number_5, Number_6, Number_7, Number_8,
			Number_9, Number_0, Number_A, Number_b, Number_C, Number_d, Number_E, Number_F, Number_Dot, };
	unsigned int index_7LED[8] = {Digit_1, Digit_2, Digit_3, Digit_4, Digit_5, Digit_6, Digit_7, Digit_8};
	unsigned int arr[8] = {0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000},input[4];
	int neg=0,ans;
	while(1)
	{
		GPIO_PTA_DIR = 0x0FF0;
		...
		for(myi =0;myi<1500;myi++){
			for(i=0;i<8;i++){
				if((i==0&&arr[i]==Number_0)||(i==2&&arr[i]==Number_0))
					continue;
		...
		for (col=0; col<4; col++)
		{
			...
			sw = (GPIO_PTC_PADIN >>2) & 0x0000000F;
			if(sw==1){
		...
		if(opt!='a'){
			...
			while(ans>0){
				tmp1=ans%10;
				if(tmp1==0)
					arr[set]=index_7LED_NUM[9];
				else
					arr[set]=index_7LED_NUM[tmp1-1];
