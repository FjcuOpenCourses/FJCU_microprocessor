...
int main()
{
	...
	unsigned int student[8] = {Number_4, Number_0, Number_7, Number_2, Number_6, Number_2, Number_6, Number_6};
				...
				for(i=0; i<(8-j); ++i)
				{
					for (myi=0; myi<1000; myi++)
					{
						for(k=0; k<8; ++k)
						{
							...
							if(k==i)GPIO_PTD_GPIO = student[7-j];

							else if(k>7-j)GPIO_PTD_GPIO = student[k];

							else GPIO_PTD_GPIO = 0x0000;
			...
			for(j=0; j<8; ++j)
			{
				for(i=0; i<(8-j); ++i)
				{
					for (myi=0; myi<1000; myi++)
					{
						for(k=0; k<8; ++k){
							GPIO_PTA_GPIO = index_7LED[k];
							if(k==i)GPIO_PTD_GPIO = student[j];
							
							else if(k>7-j)GPIO_PTD_GPIO = student[7-k];
